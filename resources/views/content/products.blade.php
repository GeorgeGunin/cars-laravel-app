    ...
    <div class="col-md-3 col-sm-6">
      ...
      <div class="btn-group">
        ...
        <ul class="dropdown-menu">
          ...
          <li><a href="{{url('shop'.'/'.$products[0]->category->curl.'/'.'orderBy'.'/'.'high-to-low')}}">High to low</a></li>
          <li><a href="{{url('shop'.'/'.$products[0]->category->curl.'/'.'orderBy'.'/'.'low-to-high')}}">Low to high</a></li>
      ...
      <div class="row">
        ...
        <div class="col-lg-6 col-md-6 col-sm-12 product">
          <div class="tile cprod">
            ...
            <a href="{{url('shop'.'/'.$product->category->curl.'/'.$product->purl)}}"><img @if($product->pimage == 'No_image_available.svg.png') style="width: 68.5%" @else style="width: 100%" @endif src="{{ asset('images'.'/'.$product->pimage) }}" alt="{{$product->ptitle}} pic"/></a>
            <div class="footer">
              <a href="{{url('shop'.'/'.$product->category->curl.'/'.$product->purl)}}">{{$product->ptitle}}</a>
            ...
            <span class=" btn add-cart-btn" data-curl="{{$product->category->curl}}"  data-id='{{$product->id}}'><span>To cart</span><i class="icon-shopping-cart"></i></span>
            @else
            ...
            <span class="wishlist-btn">
              <a  href="{{url('shop').'/'.'add-to-wishlist'.'?pid='.$product->id.'&curl='.$product->category->curl}}">
